...
2. `@DirtiesContext` : 어플리케이션 컨텍스트의 상태를 변경하는 경우 (클래스 레벨 · 메서드 레벨) 에 사용한다. 변경한 컨텍스트가 다른 테스트에 영향을 주지 않게 하기 위해, 테스트 메서드를 수행하고 나면 매번 새로운 어플리케이션 컨텍스트를 만들어서 다음 테스트가 사용하게 해준다.
